...
# Set to "all" (2026-08-06, user's explicit direction): a "Roof Soffit"-categorized ceiling in
# this project (C406965/S001) is, per the user, meant to be treated as a regular ceiling for
# generation purposes - the same "Revit's own category label doesn't match design intent" pattern
# already hit once before (Project8's mismodeled C396998, recreated as a plain ceiling). Rather
# than repeat that recreation, this just stops the ceiling script from skipping "S"-tagged hosts,
# so they get the same (correct) horizontal-drywall/vertical-furring treatment as every other
# ceiling. The dedicated soffit flow (origin_soffit_assembly_v1.py, and this flag's "soffits"
# mode) is intentionally left untouched for a genuine soffit later - user: "let's do it at last."
PROCESS_MODE = "all"
...
# Default (True): drywall boards' long edge runs EAST-WEST (along X, tiled N-S), furring runs
# NORTH-SOUTH (Y), mains run E-W - False flips all three 90 degrees. emit_drywall reads this flag
# directly. Furring MUST run perpendicular to the boards' long edge (real construction
# requirement: a board's end-to-end butt seams need to land on a framing line for backing/
# fastening) and mains perpendicular to furring (so mains end up parallel to the boards' long
# edge) - emit_furring/emit_mains therefore read the OPPOSITE of this flag internally (see
# _run_ns in each), not the same value emit_drywall uses. An earlier version had all three read
# the SAME flag, which kept furring and drywall PARALLEL instead of perpendicular in both
# orientations - confirmed live, 2026-08-06, user-flagged via named good/bad elements (drywall
# DP-C004-001/002 correct, its own ceiling's furring ST-C004-003/006 wrong - running parallel to
# the boards' long edge instead of across it; ST-S001-014/019 on a different ceiling, correctly
# perpendicular, given as the reference for what "good" looks like) - fixed by inverting emit_
# furring/emit_mains only. Briefly set to False the same session on a misreading of "I want
# drywall placements for all in horizontal, stud are in vertical" as a request to ALSO flip
# drywall - reverted back to True immediately after: the "ORIGIN Assembly" 3D view's camera has
# screen-up pointing along world -X (confirmed via the view's own GetOrientation(), and via a
# second panel the user flagged, DP-S001-018, whose long edge runs world-X and renders as a tall
# VERTICAL strip on screen - the opposite of the standard plan-view assumption that world-Y is
# "up"/horizontal-on-plan). So world-Y IS screen-horizontal here, world-X IS screen-vertical -
# True (drywall's long edge along Y) was already correct for "drywall horizontal, studs
# vertical" before the erroneous flip; this restores it. The dead (GENERATE_SCREWS-off) screw-
# along-furring-line code in emit_drywall reads neither.
FURRING_RUN_NS = True
...
# Ceilings now follow the WALL script's layout: odd courses are staggered by ODD_ROW_SHIFT_FT so
# butt joints do not line up row-to-row, exactly as origin_wall_assembly_v4_phase2 lays out its
# faces. Diffing the two config blocks, this flag was the ONLY genuine layout difference between
# them - board size, thickness, layer counts and layer staggering are already identical.
#
# Changed 2026-09-16 on the user's direction after comparing both layouts on screen, superseding
# 2026-07-16 ("I want 4x8 drywall everywhere possible"), which set this True to maximise whole
# sheets. Cost of the change, measured on Project7: 32 -> 35 boards, 41 -> 43 cuts.
#
# Set back to True for the old whole-sheet grid.
MAXIMIZE_WHOLE_PANELS = False
...
WRITE_MANIFEST = True
MANIFEST_PATH = r"C:\Users\Origoncad\Downloads\origin_revit_drywall_scripts_v2_two_faces\origin_ceiling_manifest_notaper_noscrew_nojoint.json"
            ...
            for o in boards:
                ...
                if thin_x:
                    ...
                    if abs(ob.Max.Y - bb.Min.Y) <= tol:
                        ...
                        continue
                # Standard-sheet-size cap (user spec, 2026-08-07: "CONFIRM ALL THE PANELS NOT
                # EXCEED 4 FEET HEIGHT AND 8 FEET LENGTH", applies to every merge pass, not just
                # merge_boards_split_without_real_wall - confirmed live, 2026-08-11: absorbing a
                # small sliver into a full-size neighbor here produced DP-C005-001 at 8.333ft and
                # DP-S001-001 at 8.5ft, both over the 8ft cap, because this pass never checked the
                # merged result's size). A candidate that would push the merged board over the cap
                # is skipped in favor of a farther-but-cap-safe neighbor; if none qualify, the
                # small board is left unmerged (a real undersized offcut) rather than fabricating
                # a non-standard oversized sheet.
                merged_dx = max(bb.Max.X, ob.Max.X) - min(bb.Min.X, ob.Min.X)
                merged_dy = max(bb.Max.Y, ob.Max.Y) - min(bb.Min.Y, ob.Min.Y)
                merged_short = min(merged_dx, merged_dy)
                merged_long = max(merged_dx, merged_dy)
                if merged_short > PANEL_WIDTH_FT + 1e-6 or merged_long > PANEL_LENGTH_FT + 1e-6:
                    continue
...
def _wall_crosses_segment(wall_plans, vertical, c, lo, hi, tol=0.05):
    """Does any real wall's plan footprint (from collect_wall_plans - already excludes curtain
    walls and curtain-collinear runs) actually cross this candidate shared board boundary?
    vertical=True checks a line X=c, Y in [lo,hi]; False checks Y=c, X in [lo,hi]. A wall whose
    own extent ENDS exactly at (or before) lo, or STARTS exactly at (or after) hi, only touches
    the boundary's endpoint and does not overlap it - a genuine overlap needs the wall's extent
    to reach STRICTLY past lo AND STRICTLY before hi (confirmed live, 2026-08-06: an earlier
    version of this check used the tolerance the wrong way and treated wall "001" - which ends
    exactly at the shared boundary's own start, X=-28.5237, without extending into it at all - as
    if it crossed DP-S001-013/015's shared edge, blocking a merge the user explicitly wants)."""
    for (_poly, wbb, _axis, _f_lo, _f_hi) in wall_plans:
        wx0, wy0, wx1, wy1 = wbb
        if vertical:
            if not (wx0 - tol <= c <= wx1 + tol):
                continue
            if wy1 > lo + tol and wy0 < hi - tol:
                return True
        else:
            if not (wy0 - tol <= c <= wy1 + tol):
                continue
            if wx1 > lo + tol and wx0 < hi - tol:
                return True
    return False


def merge_boards_split_without_real_wall(cid, wall_plans, warnings):
    """Post-pass, run once per ceiling after every other pass (small-board merge included): the
    per-cell piece-splitting algorithm (see emit_drywall's wall_x_cuts/wall_y_cuts stack loop)
    isolates a wall whose reach only PARTIALLY covers a cell by first slicing the cell along the
    OTHER axis at that wall's own endpoint - correct for isolating the wall's own scope, but it
    can leave an unrelated boundary behind in the region the wall never actually reaches, which
    then persists in the final board layout even though nothing there ever needed a cut
    (confirmed live, 2026-08-06: DP-S001-013/014/015 - wall "003" only spans Y:1.66-5.41, but its
    own pre-split step left an X=-28.52 boundary sitting in the Y>5.41 region above it, where
    wall 003 doesn't reach at all - user explicitly wants these treated as one panel, "think like
    putting a single panel and making the cuts as per the wall needs"). Repeatedly finds any two
    same-layer, same-ceiling, axis-aligned-adjacent boards (matching full perpendicular range,
    like merge_undersized_ceiling_boards but with NO size floor - this merges normal-sized boards
    too) whose ENTIRE shared boundary has no real wall actually crossing it, and unions them into
    one board. Returns the count of merges performed."""
    try:
        doc.Regenerate()
    except Exception as ex:
        warnings.append("doc.Regenerate() before ceiling {} wall-aware merge failed ({})".format(cid, ex))
    tag = "CEILING=" + cid
    tol = max(CODE_GAP_FT * 4.0, 0.02)
    merged_total = 0
    while True:
        ...
        try:
            ...
            warnings.append("merge_boards_split_without_real_wall scan failed for {}: {}".format(cid, ex))
            return merged_total
        ...
        for i, b in enumerate(boards):
            ...
            for o in boards[i + 1:]:
                if o["layer"] != b["layer"]:
                    continue
                ob = o["bbox"]
                if abs(bb.Min.Z - ob.Min.Z) > tol or abs(bb.Max.Z - ob.Max.Z) > tol:
                    continue
                same_y = abs(ob.Min.Y - bb.Min.Y) <= tol and abs(ob.Max.Y - bb.Max.Y) <= tol
                same_x = abs(ob.Min.X - bb.Min.X) <= tol and abs(ob.Max.X - bb.Max.X) <= tol
                boundary = None
                if same_y and not same_x:
                    if abs(ob.Min.X - bb.Max.X) <= tol:
                        boundary = (True, bb.Max.X, min(bb.Min.Y, ob.Min.Y), max(bb.Max.Y, ob.Max.Y))
                    elif abs(bb.Min.X - ob.Max.X) <= tol:
                        boundary = (True, ob.Max.X, min(bb.Min.Y, ob.Min.Y), max(bb.Max.Y, ob.Max.Y))
                elif same_x and not same_y:
                    if abs(ob.Min.Y - bb.Max.Y) <= tol:
                        boundary = (False, bb.Max.Y, min(bb.Min.X, ob.Min.X), max(bb.Max.X, ob.Max.X))
                    elif abs(bb.Min.Y - ob.Max.Y) <= tol:
                        boundary = (False, ob.Max.Y, min(bb.Min.X, ob.Min.X), max(bb.Max.X, ob.Max.X))
                if boundary is None:
                    continue
                vertical, c, lo, hi = boundary
                # Real drywall sheets have a physical size ceiling even for custom cuts - "no real
                # wall crosses this boundary" alone isn't enough to merge. User's explicit spec
                # (2026-08-06): every panel must fit within a STANDARD sheet's own footprint - short
                # side <= PANEL_WIDTH_FT (4 ft), long side <= PANEL_LENGTH_FT (8 ft) - not just an
                # area cap: an earlier version capped only total area (<= 64 sf) and still let
                # through a 7.5x7.25ft board (54 sf, under the area cap but with NEITHER side under
                # 4ft) and several 4x9.92ft boards (long side over 8ft) - confirmed live, 2026-08-06.
                merged_x = max(bb.Max.X, ob.Max.X) - min(bb.Min.X, ob.Min.X)
                merged_y = max(bb.Max.Y, ob.Max.Y) - min(bb.Min.Y, ob.Min.Y)
                merged_short = min(merged_x, merged_y)
                merged_long = max(merged_x, merged_y)
                if (merged_short > PANEL_WIDTH_FT + 1e-6 or merged_long > PANEL_LENGTH_FT + 1e-6):
                    continue
                if _wall_crosses_segment(wall_plans, vertical, c, lo, hi):
                    continue                    # a real wall actually spans this boundary - keep split
                b_solids = _ds_solids(b["ds"])
                o_solids = _ds_solids(o["ds"])
                if not b_solids or not o_solids:
                    continue
                union_solid = None
                for s1 in b_solids:
                    for s2 in o_solids:
                ...
                if union_solid is None:
                    continue
                expect_vol = sum(s.Volume for s in b_solids) + sum(s.Volume for s in o_solids)
                if union_solid.Volume < expect_vol * 0.98:
                    continue          # not a clean, non-overlapping rectangle union - leave alone
                try:
                    shape = List[GeometryObject]()
                    shape.Add(union_solid)
                    b["ds"].SetShape(shape)
                    ...
                    warnings.append("{}: wall-aware merge with {} failed ({})".format(
                        b["mark"], o["mark"], ex))
                    continue
                break
            if did_merge:
...
def ensure_export_view(warnings):
    ...
    try:
        ...
        # the wall script's copy of this function: the user couldn't find the view in a fresh
        # Revit 2027 environment because the Project Browser's active organization groups by
        ...
        # exactly as it is in the source model. Kept identical to the wall script's copy - this one
        # previously lacked even that script's Electrical Fixtures exemption, so it ran afterwards
        # and re-hid the 34 generated outlets the wall script had deliberately kept visible.
        ...
        # its whole category is hidden (confirmed live, 2026-08-06, same bug as the wall script's
        # copy of this function: real Wall/Floor/Door elements in a Revit 2027 environment reported
        # IsHidden(view)==False despite GetCategoryHidden(Walls)==True, showing through as a
        # diagonal wireframe crossing the generated drywall - user-flagged as a panel "extruding").
        # Force-hide any such element individually so the isolation is actually complete.
...
def _touches_pt(a, b, tol=0.05):
    return abs(a[0] - b[0]) <= tol and abs(a[1] - b[1]) <= tol


def _collinear_dirs(ux1, uy1, ux2, uy2, tol=0.02):
    # parallel (same OR opposite direction - a "run" doesn't care which way each segment points)
    cross = ux1 * uy2 - uy1 * ux2
    return abs(cross) <= tol


def collect_wall_plans(warnings):
    """Plan footprint of every straight wall in the model that isn't part of a curtain-wall run,
    for clipping the ceiling drywall at walls: returns a list of (poly4, bbox, axis, f_lo, f_hi)
    where axis is 'x' when the wall runs along Y (its two faces are the X lines f_lo/f_hi), 'y'
    when it runs along X, or None for an oblique wall (clip-only - no forced board joints).
    Curtain (glass) walls are excluded outright - a ceiling board can span right over a glass
    wall's plane with nothing physical to butt against, unlike a solid wall, so forcing a split/
    joint there is wrong (confirmed live, 2026-08-06, user report: DP-S001-003/004 needlessly
    split at a curtain wall segment when they should be one board - same curtain walls already
    excluded from getting their own drywall/framing on the wall side, see _is_curtain_wall in the
    wall assembly script). ALSO excluded: any real (solid) wall that is collinear with, and
    touches at an endpoint, an actual curtain wall - a short solid infill/spandrel segment sitting
    within an otherwise glazed facade run is functionally part of that same curtain-wall system,
    not an interior partition, and forcing an ADDITIONAL board division at its own short span
    produces the same visually-unwanted fragmentation the curtain-wall exclusion above already
    fixes for the glazed segments themselves (confirmed live, 2026-08-06: wall "016", a solid 6in
    stub only 1.667 ft long sitting exactly between curtain segments 031/032 on the same facade
    line, narrowed DP-S001-004 relative to its neighbors DP-S001-001/003 even after the curtain-
    wall fix - user explicitly wants these merged, "it's needed visually" - while confirming real,
    genuinely interior walls should keep splitting boards exactly like before)."""
    plans = []
    try:
    ...
    curtain_segs = []
    for cw in all_walls:
        if not _is_curtain_wall(cw):
            continue
        try:
            crv = cw.Location.Curve
            if not isinstance(crv, Line):
                continue
            cp0, cp1 = crv.GetEndPoint(0), crv.GetEndPoint(1)
            cdx, cdy = cp1.X - cp0.X, cp1.Y - cp0.Y
            cln = math.sqrt(cdx * cdx + cdy * cdy)
            if cln < 1e-6:
                continue
            curtain_segs.append(((cp0.X, cp0.Y), (cp1.X, cp1.Y), cdx / cln, cdy / cln))
        except Exception:
            continue

    def _is_curtain_run_wall(w):
        ...
        try:
            ...
            return False
        for (cp0, cp1, cux, cuy) in curtain_segs:
            if not _collinear_dirs(wux, wuy, cux, cuy):
                continue
            if (_touches_pt((p0.X, p0.Y), cp0) or _touches_pt((p0.X, p0.Y), cp1) or
                    _touches_pt((p1.X, p1.Y), cp0) or _touches_pt((p1.X, p1.Y), cp1)):
                return True
        return False

    for w in all_walls:
        if _is_curtain_run_wall(w):
            continue
        try:
            loc = w.Location
...
def emit_furring(ceiling_id, host_tag, st_counter, z_top, boundary_solid, xlo, xhi, ylo, yhi,
    ...
    """Furring hat-channels @16" OC, just above the drywall (bottom face at z_top). Runs
    PERPENDICULAR to the drywall boards' own long edge (real construction requirement - a board's
    end-to-end butt seams need to land on a framing line for backing/fastening), so this reads
    _run_ns = NOT FURRING_RUN_NS (the opposite of what emit_drywall reads - see the flag's own
    module-level comment for the full reasoning). Long lines are spliced into stock-length pieces
    that lap at each joint (splice_segments), with the splices staggered row-to-row. Each piece is
    clipped to the ceiling outline and named ST-<host>-<n> (flat per-ceiling framing sequence).
    Returns (pieces_made, splice_joints)."""
    ...
    _run_ns = not FURRING_RUN_NS
    if _run_ns:
        space_lo, space_hi = xlo, xhi
        run_lo, run_hi = ylo, yhi
    else:
        space_lo, space_hi = ylo, yhi
        run_lo, run_hi = xlo, xhi
    s = space_lo
    while s <= space_hi + 1e-6:
        if (run_hi - run_lo) >= MIN_SEG_FT:
            # A long furring line ships as stock-length pieces lapped at each splice; stagger the
            # splices row-to-row so joints don't line up. cross-section is across the run axis and
            # up Z, centered on the furring line at s; extrude along the run axis.
            segs = splice_segments(run_lo, run_hi, FURRING_STOCK_LENGTH_FT, FURRING_SPLICE_LAP_FT,
            ...
            for si, (a, b) in enumerate(segs):
                ...
                if _run_ns:
                    origin = XYZ(s - FURRING_FACE_FT / 2.0, a, z_top + dz)
                    raw = make_profile_solid(origin, XYZ.BasisX, XYZ.BasisZ, prof, XYZ.BasisY, seg_len, mat, gs)
                else:
                    origin = XYZ(a, s - FURRING_FACE_FT / 2.0, z_top + dz)
                ...
                if ds:
                    ...
                    rec = {
                        ...
                        "length_in": round(seg_len * 12.0, 3),
                        "piece": si + 1, "pieces": len(segs),
                        "bbox_ft": fbox, "room_id": room_id,
                        "install_order": len(frecords) + 1,
                    }
                    if _run_ns:
                        rec["line_x_in"] = round(s * 12.0, 2)
                    else:
                        rec["line_y_in"] = round(s * 12.0, 2)
                    frecords.append(rec)
        fi += 1
        s += FURRING_SPACING_FT
    return made, splices
...
def emit_mains(ceiling_id, host_tag, st_counter, z_top, boundary_solid, outer, holes, fixtures,
    ...
    """Reinforcement for long spans: when the furring run exceeds MAX_FURRING_SPAN_FT, the furring
    can't carry the gypsum without sagging, so add MAIN carrying channels perpendicular to the
    furring at MAIN_SPACING_FT (sitting just above the furring), each hung from the structure by
    HANGER wires at HANGER_SPACING_FT. Furring then spans <= MAX_FURRING_SPAN_FT between mains.
    Mains must stay perpendicular to furring's OWN run direction (so they end up parallel to the
    drywall boards' long edge - see emit_furring's docstring for the full reasoning), so this
    reads the same _run_ns = NOT FURRING_RUN_NS emit_furring uses, not FURRING_RUN_NS directly.
    Mains are clipped to the ceiling outline; hangers are placed only where they land inside the
    room. Mains ship in stock lengths too, so long ones are spliced (lapped) and staggered between
    mains. Named ST-<host>-<n> (shared framing sequence). Returns (mains_made, hangers_made,
    splice_joints)."""
    _run_ns = not FURRING_RUN_NS
    if _run_ns:
        span_lo, span_hi = ylo, yhi          # furring's own run length (now along Y)
        space_lo, space_hi = ylo, yhi        # mains spaced along Y (furring's run axis)
        run_lo, run_hi = xlo, xhi            # mains run along X (furring's spacing axis)
    else:
        span_lo, span_hi = xlo, xhi          # furring's own run length (along X, default)
        space_lo, space_hi = xlo, xhi        # mains spaced along X
        run_lo, run_hi = ylo, yhi            # mains run along Y
    if not GENERATE_MAINS or (span_hi - span_lo) <= MAX_FURRING_SPAN_FT + 1e-6:
        return 0, 0, 0
    ...
    # Interior main positions every MAIN_SPACING; the furring is also supported at the two ends
    # (perimeter), so only interior supports are needed.
    m = space_lo + MAIN_SPACING_FT
    mi = 0
    while m < space_hi - 1e-6:
        if (run_hi - run_lo) >= MIN_SEG_FT:
            # A long carrying channel also ships in stock lengths, lapped at each splice; stagger
            # the splices between adjacent mains.
            segs = splice_segments(run_lo, run_hi, MAIN_STOCK_LENGTH_FT, MAIN_SPLICE_LAP_FT,
            ...
            for si, (a, b) in enumerate(segs):
                ...
                if _run_ns:
                    origin = XYZ(a, m - MAIN_FACE_FT / 2.0, z_main + dz)
                    raw = make_profile_solid(origin, XYZ.BasisY, XYZ.BasisZ, prof, XYZ.BasisX, seg_len, mat, gs)
                else:
                    origin = XYZ(m - MAIN_FACE_FT / 2.0, a, z_main + dz)
                    raw = make_profile_solid(origin, XYZ.BasisX, XYZ.BasisZ, prof, XYZ.BasisY, seg_len, mat, gs)
                clipped = boolean_op(raw, boundary_solid, BooleanOperationsType.Intersect)
                if not solid_ok(clipped):
                    continue
                ...
                if ds:
                    ...
                    if _run_ns:
                        rec["line_y_in"] = round(m * 12.0, 2)
                    else:
                        rec["line_x_in"] = round(m * 12.0, 2)
                    frecords.append(rec)
            ...
            if made_here and GENERATE_HANGERS:
                hpos = [run_lo + END_SUPPORT_SETBACK_FT]
                hh = run_lo + END_SUPPORT_SETBACK_FT + HANGER_SPACING_FT
                while hh < run_hi - END_SUPPORT_SETBACK_FT + 1e-6:
                    hpos.append(hh)
                    hh += HANGER_SPACING_FT
                hpos.append(run_hi - END_SUPPORT_SETBACK_FT)
                half = HANGER_DIA_FT / 2.0
                for hp in unique_sorted(hpos):
                    px, py = (hp, m) if _run_ns else (m, hp)
                    if not point_inside_room(px, py, outer, holes, fixtures):
                        continue
                    hrect = (px - half, py - half, px + half, py + half)
                    ...
                    if hds:
                        ...
                        frecords.append({
                            ...
                            "x_in": round(px * 12.0, 2), "y_in": round(py * 12.0, 2),
...
def emit_drywall(ceiling_id, host_tag, z_bottom, outer, holes, fixtures, boundary_solid, rated,
    ...
    # Panel grid reads FURRING_RUN_NS directly (the framing reads its OPPOSITE - see emit_furring's
    # docstring): by default courses step in Y (panels' long edge along X); flipped, courses step
    # in X (panels' long edge along Y).
    if FURRING_RUN_NS:
        course_lo, course_hi = bx0, bx1
        span_lo, span_hi = by0, by1
        full_dx, full_dy = PANEL_WIDTH_FT, PANEL_LENGTH_FT
    else:
        course_lo, course_hi = by0, by1
        span_lo, span_hi = bx0, bx1
        full_dx, full_dy = PANEL_LENGTH_FT, PANEL_WIDTH_FT
    ...
    for li, (t, type_x) in enumerate(layers):
        ...
        # courses step along the course axis by PANEL_WIDTH, offset by v_off for layer staggering
        cstart = course_lo + v_off
        while cstart > course_lo:
            cstart -= PANEL_WIDTH_FT
        row = 0
        cc = cstart
        while cc < course_hi - 1e-4:
            cc0 = max(course_lo, cc)
            cc1 = min(course_hi, cc + PANEL_WIDTH_FT)
            cc += PANEL_WIDTH_FT
            row += 1
            if (cc1 - cc0) < MIN_PIECE_HEIGHT_FT:
                continue
            full_course = abs((cc1 - cc0) - PANEL_WIDTH_FT) < 0.02
            ...
            while s < span_hi - 1e-4:
                ...
                col += 1
                if FURRING_RUN_NS:
                    rx0, cy0, rx1, cy1 = cc0, rs0, cc1, rs1
                else:
                    rx0, cy0, rx1, cy1 = rs0, cc0, rs1, cc1
                    ...
                    if whole:
                        for wb in wall_bboxes:          # near/over a wall -> take the boolean path so
                            if rects_overlap(rect, wb):  # the wall subtraction in the mask trims it
                                whole = False
                                break
                    is_full_size = (abs((sx1 - sx0) - (full_dx - CODE_GAP_FT)) < 0.02 and
                                    abs((sy1 - sy0) - (full_dy - CODE_GAP_FT)) < 0.02)
        ...
        for ceiling in ceilings:
            ...
            try:
                ...
                if do_drywall:
                    ...
                    wall_aware_merged = merge_boards_split_without_real_wall(cid, wall_plans, warnings)
                    if wall_aware_merged:
                        result["boards_created"] -= wall_aware_merged
                        result["small_boards_merged"] += wall_aware_merged
