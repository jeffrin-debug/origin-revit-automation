# Origin Ceiling Assembly Generator v1 - FRAMING + DRYWALL (FURRED / SUSPENDED)  [NO-SCREWS BUILD]
...
#
# NO-SCREWS VARIANT: identical to origin_ceiling_assembly_v1.py (same APP_ID, same nomenclature,
# same cleanup) EXCEPT it never generates screw markers or screw records - useful for a lighter
# Isaac Sim model or when fasteners aren't needed yet. Kept in lockstep with the trunk; the ONLY
# differences are this header, GENERATE_SCREWS = False, and MANIFEST_PATH. Because it shares the
# trunk's APP_ID + CEILING=C<eid> cleanup token, running this on a ceiling REPLACES the trunk's
# assembly on that ceiling with a screwless one (and vice-versa) - interchangeable alternates, not
# additive. (The trunk can also just be run with Dynamo IN[8]=False to skip screws.)
#
# WHAT THIS IS:
...
GENERATE_SCREWS = False     # NO-SCREWS BUILD: hard off (Dynamo IN[8] can still force it on)
COLOR_BY_TYPE = True
...
WRITE_MANIFEST = True
MANIFEST_PATH = r"C:\Users\Origoncad\Downloads\origin_revit_drywall_scripts_v2_two_faces\origin_ceiling_manifest_noscrews.json"
