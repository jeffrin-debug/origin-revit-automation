...
def ensure_export_view(warnings):
    ...
    try:
        ...
        # view. Guarantees the old wall is not seen in the sim. Derives the set of categories to
        # hide from elements ACTUALLY PRESENT in the document rather than iterating
        # doc.Settings.Categories, which is INCOMPLETE in this Revit 2027 environment (confirmed
        # live, 2026-08-06: "Roof Soffits" - a real, hideable, Model-type category with real
        # content in this document - never appeared in that enumeration at all, so it was silently
        # never hidden even though the identical category IS directly accessible/hideable via
        # BuiltInCategory.OST_RoofSoffit lookup; its Ceiling instance showed through as a large
        # extruding shape crossing the generated drywall).
        ...
        try:
            for e in FilteredElementCollector(doc).WhereElementIsNotElementType():
                try:
                    cat = e.Category
                    if cat is None or cat.CategoryType != CategoryType.Model or cat.Id == gm_id or cat.Id == ef_id:
                        continue
        ...
        # its whole category is hidden (confirmed live, 2026-08-06: real Wall/Floor/Door elements
        # in this Revit 2027 environment reported IsHidden(view)==False despite
        # GetCategoryHidden(Walls)==True, showing through as a diagonal wireframe crossing the
        # generated drywall - user-flagged as a panel "extruding"). Force-hide any such element
        # individually so the isolation is actually complete regardless of its prior override.
        try:
...
def trim_drywall_from_neighbor_studs(warnings):
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            ...
            if bi["host"] == bj["host"]:
                continue
            a, b2 = (bi, bj) if bi["mark"] < bj["mark"] else (bj, bi)
...
def emit_corner_infill_for_exposed_studs(warnings, board_records):
    ...
    for s in studs:
        ...
        for face_config in FACE_LAYOUTS:
            ...
            candidate = make_rect_solid(layer_origin, u, XYZ.BasisZ, normal,
                                        (lx0, lz0, lx1, lz1), t, mat_id, gs_id)
...
if len(walls) == 0:
    ...
    try:
        ...
        if GENERATE_DRYWALL and GENERATE_FRAMING:
            ...
            result["corner_infill_boards"] = emit_corner_infill_for_exposed_studs(warnings, board_records)
